...
Lemma sim_fwd_view1 tid ts mem1 mem2 loc coh1 coh2 fwd1 fwd2 o
  ...
  sim_view ts mem1 mem2 (FwdItem.read_view fwd1 fwd1.(FwdItem.ts) o) (FwdItem.read_view fwd2 fwd2.(FwdItem.ts) o).
Proof.
...
Proof.
  ...
  { (* state step *)
    ...
    - (* read *)
      ...
      { (* Case 1: Tgt read from fwdbank. Src reads from fwdbank, too. *)
        ...
        - econs 1. econs. econs; ss.
          ...
          + rewrite ELOC in *.
            ...
            * 
              Lemma read_app1_inv ts loc val mem1 mem2
                    (TS: ts <= length mem1)
                    (READ: Memory.read loc ts (mem1 ++ mem2) = Some val):
                Memory.read loc ts mem1 = Some val.
              Proof.
                revert READ. unfold Memory.read. destruct ts; ss.
                destruct (nth_error (mem1 ++ mem2) ts) eqn:NTH; ss.
                rewrite nth_error_app1 in NTH; ss. rewrite NTH. ss.
              Qed.
              Lemma sim_fwdbank_read
                    ts mem1 mem2 loc fwd1 fwd2
                    (SIM: sim_fwdbank ts mem1 mem2 loc fwd1 fwd2):
                Memory.read loc fwd1.(FwdItem.ts) mem1 = Memory.read loc fwd2.(FwdItem.ts) mem2.
              Proof.
                inv SIM; ss.
              Qed.

              erewrite sim_fwdbank_read; eauto.
      ...
      (* Now tgt didn't read from fwdbank. *)
      ...
      { (* Case 2: Tgt didn't read from fwdbank, and tgt's post-view <= ts. Src reads the same msg. *)
        rename l into POST_BELOW. inv LOCAL.
      ...
      { (* Case 3: Tgt didn't read from fwdbank, and tgt's post-view > ts. Src reads the latest msg. *)
